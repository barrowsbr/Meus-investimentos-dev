...
export default function PatrimonioPage() {
  ...
  if (!serie || !m || serie.pontos.length === 0) {
    return (
      <>
        <PageHeader title="Patrimônio" description="Histórico do patrimônio total" />
  ...
  return (
    <>
      <PageHeader title="Patrimônio" description={`Histórico do patrimônio total · aba historico_patrimonio · ${serie.pontos.length} registros`} />

      {/* Resumo */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5 mt-4">
        <MetricCard
          label="Patrimônio atual"
          value={compactBRL(m.atual.total)}
          sub={m.atual.label}
          icon={<Wallet size={18} />}
        />
        <MetricCard
