...
export default function ResumoPage() {
  ...
  const avgMonthlyDividend = useMemo(() => {
    if (monthlyDividends.length === 0) return 0;
    return (
      monthlyDividends.reduce((s: number, m: { month: string; total: number }) => s + m.total, 0) /
      monthlyDividends.length
    );
  }, [monthlyDividends]);
  ...
  return (
    <>
      <PageHeader title="Resumo" description="Visão geral dos seus investimentos" />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 mb-8">
        <div className="animate-fade-in">
          <MetricCard
            ...
            glowColor="#d4a574"
            borderGradient={CARD_GRADIENTS.patrimonio}
          />
        </div>
        <div className="animate-fade-in animate-delay-1">
          <MetricCard
            ...
            icon={<BarChart3 size={17} strokeWidth={1.6} />}
            glowColor="#3b82f6"
            borderGradient={CARD_GRADIENTS.rv}
          />
        </div>
        <div className="animate-fade-in animate-delay-2">
          <MetricCard
            ...
            icon={<Landmark size={17} strokeWidth={1.6} />}
            glowColor="#8b5cf6"
            borderGradient={CARD_GRADIENTS.rf}
          />
        </div>
          ...
          <MetricCard
            ...
            icon={
              data.lucroBRL >= 0 ? (
                ...
                <TrendingDown size={17} strokeWidth={1.6} />
              )
            }
            trend={data.lucroBRL >= 0 ? "up" : "down"}
            glowColor={data.lucroBRL >= 0 ? "#10b981" : "#f87171"}
            borderGradient={
              data.lucroBRL >= 0 ? CARD_GRADIENTS.lucroUp : CARD_GRADIENTS.lucroDown
            }
          />
        </div>
        <div className="animate-fade-in animate-delay-4">
          <MetricCard
            ...
            icon={<Coins size={17} strokeWidth={1.6} />}
            glowColor="#f59e0b"
            borderGradient={CARD_GRADIENTS.proventos}
          />
        </div>
        <div className="animate-fade-in animate-delay-5">
          <MetricCard
            ...
            glowColor="#10b981"
            borderGradient={CARD_GRADIENTS.dolar}
          />
        </div>
      </div>
              ...
              <thead>
                <tr className="border-b text-left" style={{ borderColor: "#1E2028" }}>
                  {["Ativo", "Setor", "Qtd", "Preço", "Valor", "Lucro", "%"].map((h, i) => (
                    <th
                      key={h}
                      className={`px-3 py-2.5 text-[10px] text-zinc-500 font-semibold uppercase tracking-wider ${i > 1 ? "text-right" : ""}`}
                    >
                      {h}
                    </th>
                  ))}
