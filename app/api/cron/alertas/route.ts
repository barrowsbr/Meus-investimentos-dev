...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (const t of toSend) {
      // Mesmo alerta para dono + convidados (helper único de broadcast).
      const envio = await paraTodos(config, (destino) => sendTelegramMessage(token, destino, t.texto));
      const res = { ok: envio.enviados > 0, error: envio.erro };
