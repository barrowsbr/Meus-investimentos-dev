...
export default function MercadosPreditivosPage() {
    ...
    (async () => {
      ...
      const [polyRes, kalshiRes, metaculusRes] = await Promise.allSettled([
        ...
        fetchMetaculus().then(qs => qs.map(q => ({
          id: q.id, source: q.source, title: q.title, url: q.url,
          category: q.category, odds: q.odds, forecasters: q.forecasters,
          end_date: q.end_date, days_left: q.days_left, portfolio_impact: q.portfolio_impact,
        } as UnifiedPrediction))),
      ]);

      if (polyRes.status === "fulfilled") {
        allPreds.push(...polyRes.value);
        counts.polymarket = polyRes.value.length;
      }
      if (kalshiRes.status === "fulfilled") {
        allPreds.push(...kalshiRes.value.map(k => ({ ...k } as UnifiedPrediction)));
        counts.kalshi = kalshiRes.value.length;
      }
      if (metaculusRes.status === "fulfilled") {
        allPreds.push(...metaculusRes.value);
        counts.metaculus = metaculusRes.value.length;
      }
  ...
  const categorized = useMemo(() => {
    ...
    // First pass: portfolio-correlated
    const portfolioKeywords: string[] = [];
    for (const t of portfolioTickers) {
      const clean = t.replace(".SA", "").toLowerCase();
      portfolioKeywords.push(clean);
    }

    const correlated: UnifiedPrediction[] = [];
    for (const p of items) {
      if (p.portfolio_impact.length > 0) correlated.push(p);
    }
    ...
    // Sort within categories by volume/forecasters
    for (const [, evs] of cats) {
      evs.sort((a, b) => (b.volume ?? b.forecasters ?? 0) - (a.volume ?? a.forecasters ?? 0));
    }
    ...
    return sorted;
  }, [predictions, sourceFilter, portfolioTickers]);

  const totalMarkets = useMemo(() => categorized.reduce((s, [, evs]) => s + evs.length, 0), [categorized]);

  const filteredCategories = useMemo(() => {
    if (!activeFilter) return categorized;
    return categorized.filter(([name]) => name === activeFilter);
    ...
    <>
      <div className="flex items-start justify-between mb-4">
        <PageHeader
          ...
          description="Polymarket, Kalshi e Metaculus — apostas e previsões com impacto econômico"
        />
      ...
      {error && (
        <div className="mb-4 p-4 bg-red-500/10 border border-red-500/20 rounded-2xl text-sm text-red-400">{error}</div>
      )}
      ...
      {!loading && predictions.length > 0 && (
        <>
          <div className="flex flex-wrap items-center gap-3 mb-4 text-[11px] text-zinc-600">
            <span className="flex items-center gap-1.5"><Activity size={12} className="text-cyan-400" /><strong className="text-zinc-300">{predictions.length}</strong> mercados de {Object.values(sourceCounts).filter(v => v > 0).length} fontes</span>
            ...
            <span className="flex items-center gap-1"><Briefcase size={10} className="text-amber-400" />{predictions.filter(p => p.portfolio_impact.length > 0).length} correlatas ao portfólio</span>
          </div>
              ...
              return (
                <button key={s} onClick={() => { setSourceFilter(s); setActiveFilter(null); }}
                  className={`flex-shrink-0 flex items-center gap-1.5 py-1 px-3 rounded-xl text-xs font-semibold border transition-all ${
                    sourceFilter === s ? "bg-white/[0.08] border-white/[0.15] text-zinc-200" : "border-white/[0.06] text-zinc-500 hover:text-zinc-300 hover:bg-white/[0.04]"
                  }`}>
                  {s === "all" ? <Globe size={11} /> : <Activity size={11} />}
                  {s === "all" ? "Todas as Fontes" : badge?.label ?? s}
            ...
            <div className="flex gap-1.5 mb-5 overflow-x-auto pb-1">
              <button onClick={() => setActiveFilter(null)}
                className={`flex-shrink-0 flex items-center gap-1.5 py-1.5 px-3 rounded-xl text-xs font-semibold border transition-all ${
                  !activeFilter ? "bg-white/[0.08] border-white/[0.15] text-zinc-200" : "border-white/[0.06] text-zinc-500 hover:text-zinc-300 hover:bg-white/[0.04]"
                }`}>
              ...
              {categorized.map(([name, events]) => {
                ...
                return (
                  <button key={name} onClick={() => setActiveFilter(activeFilter === name ? null : name)}
                    className={`flex-shrink-0 flex items-center gap-1.5 py-1.5 px-3 rounded-xl text-xs font-semibold border transition-all ${
                      activeFilter === name ? "bg-white/[0.08] border-white/[0.15] text-zinc-200" : "border-white/[0.06] text-zinc-500 hover:text-zinc-300 hover:bg-white/[0.04]"
                    }`}>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-zinc-600">
          ...
          <p className="text-sm">Não foi possível carregar mercados preditivos.</p>
        </div>
      ) : (
            ...
            return (
              <div key={name}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {events.map(ev => <UnifiedCard key={`${ev.source}-${ev.id}`} pred={ev} />)}
                </div>
              </div>
            );
          })}
