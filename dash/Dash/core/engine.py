...
def reconstruct_history_multicurrency(
    ...
    # 3. Extract FX Rates
    # USD/BRL comes directly from Yahoo as BRL=X
    s_usd = df_prices['BRL=X'] if 'BRL=X' in df_prices.columns else pd.Series(5.5, index=idx_dates)
    
    # EUR/BRL and CAD/BRL need to be calculated as cross-rates
    # EUR/BRL = EUR/USD * USD/BRL
    # CAD/BRL = CAD/USD * USD/BRL
    
    if 'EURUSD=X' in df_prices.columns:
        s_eur = df_prices['EURUSD=X'] * s_usd  # EUR/USD * USD/BRL = EUR/BRL
    elif 'EURBRL=X' in df_prices.columns:
        s_eur = df_prices['EURBRL=X']
    else:
        s_eur = pd.Series(6.3, index=idx_dates)  # Fallback atualizado
    
    if 'CADUSD=X' in df_prices.columns:
        s_cad = df_prices['CADUSD=X'] * s_usd  # CAD/USD * USD/BRL = CAD/BRL
    elif 'CADBRL=X' in df_prices.columns:
        s_cad = df_prices['CADBRL=X']
    else:
        s_cad = pd.Series(4.2, index=idx_dates)  # Fallback atualizado
    ...
    if has_rv:
        ...
        # FIX v18.0: Filter RF-like tickers from currency_groups
        # These are handled exclusively by the RF engine (Section 10)
        # Including them here causes double-counting in the final NAV
        df_rv_only = df_bruto[~df_bruto['ticker'].str.upper().str.contains(
            '|'.join(termos_excluir), na=False
        )]
        currency_groups = df_rv_only.groupby('moeda')['ticker'].apply(lambda x: list(set(x))).to_dict()
    else:
        currency_groups = {}

    # =====================================================================
    # FIX: Separate tickers bought directly in BRL (e.g., BTC-USD)
    # These use market FX rate, not remittance cost basis in "My Money" mode
    # =====================================================================
    DIRECT_BRL_TICKERS = ['BTC-USD', 'BTC']
    for curr_key in list(currency_groups.keys()):
        if curr_key == 'BRL':
            continue
        tickers_in_group = currency_groups[curr_key]
        excluded = [t for t in tickers_in_group if t.upper() in [x.upper() for x in DIRECT_BRL_TICKERS]]
        remaining = [t for t in tickers_in_group if t.upper() not in [x.upper() for x in DIRECT_BRL_TICKERS]]

        if excluded:
            direct_key = f"{curr_key}_DIRECT"
            currency_groups[direct_key] = excluded
            if remaining:
                currency_groups[curr_key] = remaining
            else:
                del currency_groups[curr_key]
            # Add FX rate for DIRECT bucket (same as base currency)
            if curr_key in fx_rates:
                fx_rates[direct_key] = fx_rates[curr_key]
    ...
    if has_rv:
        ...
        # Update moeda for DIRECT tickers (e.g., BTC-USD -> USD_DIRECT)
        for ticker in DIRECT_BRL_TICKERS:
            mask = df_ops['ticker'].str.upper() == ticker.upper()
            if mask.any():
                original_moeda = df_ops.loc[mask, 'moeda'].iloc[0]
                if original_moeda != 'BRL':
                    df_ops.loc[mask, 'moeda'] = f"{original_moeda}_DIRECT"
    ...
    if has_rf:
        try:
            ...
            # =====================================================================
            # FIX v19.0: Filtrar CAIXA/SALDO/CASH de df_rf_proc ANTES do engine
            #
            # PROBLEMA:
            # - CAIXA existe em duas abas do GSheets (RF transações e RF manual)
            # - Se Ticker=CAIXA tem Tipo=Compra, entra como lote normal em 'corrected'
            # - cash_balance é extraído separadamente de RF manual
            # - Resultado: CAIXA contado duas vezes (~R$ 4k extra)
            #
            # SOLUÇÃO:
            # - Filtrar tickers CAIXA/SALDO/CASH de df_rf_proc
            # - CAIXA só é usado via cash_balance (de Performance_Advanced._load_all_data)
            # =====================================================================
            CASH_TICKERS = ['CAIXA', 'SALDO', 'CASH']
            if 'Ticker' in df_rf_proc.columns:
                # DEBUG v19.0: Log quantas linhas de CAIXA foram filtradas
                mask_cash = df_rf_proc['Ticker'].astype(str).str.strip().str.upper().isin(CASH_TICKERS)
                n_cash_removed = mask_cash.sum()
                if n_cash_removed > 0:
                    cash_value = df_rf_proc.loc[mask_cash, 'Valor'].sum() if 'Valor' in df_rf_proc.columns else 0
                    print(f"[FIX v19.0] Removendo {n_cash_removed} linhas de CAIXA do RF (valor total: R$ {cash_value:,.2f})")
                df_rf_proc = df_rf_proc[~mask_cash]
            ...
            if not rf_result.daily_curve.empty:
                # FIX v18.0: Usar 'corrected' ao invés de 'total' para excluir CAIXA
                # O caixa é tratado separadamente e não deve ser incluído no NAV do portfólio
                # 'total' = corrected + cash, 'corrected' = apenas ativos de RF
                rf_curve = rf_result.daily_curve['corrected']
                ...
                # =====================================================================
                # FIX v17.0: Force final RF NAV to match sum of manual values
                # This ensures Performance shows EXACTLY the same RF value as Composição
                #
                # PROBLEMA:
                # - FixedIncomeEngine calcula RF via taxa SELIC proxy (~15% a.a.)
                # - Composição usa valores manuais reais da planilha
                # - Diferença ~R$5k entre Performance (R$195k) e Composição (R$190k)
                #
                # SOLUÇÃO:
                # - Sobrescrever o último valor de rf_nav com a soma dos manual values
                # - Curva histórica preservada para cálculo de TWR
                # =====================================================================
                ...
                # O FixedIncomeEngine já usa manual_values internamente para calcular
                # a taxa implícita. Escalar aqui duplicava o efeito.
                # =====================================================================
                ...
                # Parâmetros de detecção de fluxos (v13.0 - thresholds reduzidos)
                SELIC_DAILY_RATE = 0.15 / 252  # ~15% a.a.
                FLOW_PERCENT_THRESHOLD = 0.005  # 0.5% do NAV (era 1%)
                FLOW_MIN_ABSOLUTE = 10.0  # R$10 mínimo (era R$50)
            ...
            import traceback
            print(f"[RF ENGINE ERROR] Exception: {e}")
            traceback.print_exc()
            pass
