...
import { fxToBRL, type FxRates } from "./cotacoes";

// ─── Motor canônico de Renda Fixa manual (CDB/Tesouro/caixa) ─────────────────
// Fonte única do resultado de RF manual (CANONICO.md §2): consumido pela rota
// /api/renda-fixa/posicoes (que alimenta as páginas Renda Fixa e Resumo) e
// pelo contexto do agente IA. Nunca reimplementar este cálculo em outro lugar.
...
export interface RFOpenPosition {
  ticker: string;
  moeda: string;
  atual: number;
  atualBRL: number;
...
export interface RFClosedPosition {
  ticker: string;
  moeda: string;
  compra: number;
  venda: number;
...
export interface RendaFixaResult {
  abertas: RFOpenPosition[];
  caixa: RFOpenPosition[];
...
export function calcularRendaFixaPosicoes(
  rfTransacoes: Row[],
  fixaAberta: Row[],
  proventosRows: Row[],
  fx: FxRates,
  ...
  // 7. All transactions for display
  const transacoes: RFTransaction[] = Object.values(txByTicker).flatMap(t => t.txs);
