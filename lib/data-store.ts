...
export interface DataStore {
  ...
  appendRowsTyped(tabName: string, rows: (string | number)[][]): Promise<void>;
...
class GSheetsDataStore implements DataStore {
  ...
  async appendRowsTyped(tabName: string, rows: (string | number)[][]): Promise<void> {
    const { appendRowsTyped } = await import("./gsheets");
    return appendRowsTyped(tabName, rows);
  }
